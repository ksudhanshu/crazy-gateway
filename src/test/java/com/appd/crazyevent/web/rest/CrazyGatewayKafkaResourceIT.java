...
class CrazyGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        CrazyGatewayKafkaResource kafkaResource = new CrazyGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/crazy-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/crazy-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
